  ...
  <div class="preloader" id="preloader">
    ...
    <section class="login-content" >
        ...
        <div class="login-boxn"  >
            <form  class="login-form" action="{{route('passwordconfirmation.put',$token)}}"  method="POST">
                {{@csrf_field()}}
                {{ method_field('PUT') }}
                <h3 class="login-head">Creation de mot de passe</h3>
                @if(session()->has('error'))
                    <div class="alert alert-danger">{{session('error')}}</div>
                @endif
                @if($date_dif > 86400)
                    <div class="form-group btn-container">
                        <a href="{{ route('actualiserinscription', $user->id) }}" class="btn btn-primary btn-block" style="background: #078a36"><i class="fa fa-sign-in fa-lg fa-fw"></i>REINITIALISER VOTRE DEMANDE D'INSCRIPTION</a>
                    </div>
                @endif <br>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control @error('password_confirmation') is-invalid @enderror" value="{{ old('password_confirmation') }}" type="password" name="password_confirmation">
                    @error('password_confirmation')<span class="text text-danger">{{$message}}</span>@enderror
                </div>
                <div class="form-group">
                    <input class="form-control" value="{{ $email }}"  type="email" name="email" hidden>
                </div>
                <div class="form-group btn-container">
